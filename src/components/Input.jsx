...
export const Input = forwardRef(({ name, label, onChange, error}, ref) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <input type="number" name={name} id={`input-${name}`} onChange={onChange} placeholder={placeholder()} ref={ref} className={`text-2xl w-20 md:w-40 h-10 md:h-16 px-4 py-2 font-bold text-black ${inputBorderClass} rounded-xl outline-none focus:border-figma-purple`}/>
